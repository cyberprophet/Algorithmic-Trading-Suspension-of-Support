...
namespace ShareInvest
{
    ...
    class BringInInformation
    {
        ...
        {
            var revise = Security.Client.GetContextAsync(new RevisedStockPrice { Code = cm.Code }).Result as Queue<ConfirmRevisedStockPrice>;
        ...
        async IAsyncEnumerable<IEnumerable<Catalog.Strategics.Charts>> FindTheOldestDueDate()
        {
            if (cm.Code.Length == 8 && cm.Code[0] == '1' && cm.Code[1] == '0')
            {
                ...
                var list = await Security.Client.GetContextAsync(new Codes(), 8) as List<Codes>;
                ...
                foreach (var arg in (await Security.Client.GetChartsAsync(new Catalog.Models.Charts
                {
                ...
                while (stack.Count > 0)
                {
                    ...
                    yield return await Security.Client.GetChartsAsync(new Catalog.Models.Charts
                    {
                        Code = codes.Code,
                        Start = start.Length == 8 ? start[2..] : start,
                        End = codes.MaturityMarketCap.Length == 8 ? codes.MaturityMarketCap[2..] : codes.MaturityMarketCap
            ...
            {
                string sDate = await Security.Client.GetChartsAsync(new Catalog.Models.Charts { Code = cm.Code, Start = empty, End = empty }) as string,
                    date = string.IsNullOrEmpty(sDate) ? DateTime.Now.AddDays(-5).ToString(Base.DateFormat) : sDate.Substring(0, 6);

                foreach (var day in from day in (await Security.Client.GetChartsAsync(new Catalog.Models.Charts { Code = cm.Code, Start = string.Empty, End = string.Empty })
                                    as IEnumerable<Catalog.Strategics.Charts>).OrderBy(o => o.Date)
                ...
                if (int.TryParse(date, out int start))
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(end) == false && end[2..].CompareTo(excluding[2..]) > 0 && end[2..].CompareTo(theDate[2..]) < 0)
