...
namespace ShareInvest.Controls
{
    public partial class Yield : UserControl, IEnumerable
    {
        ...
        public IEnumerator GetEnumerator()
        {
            foreach (var temp in new Recall(code, (long)assets))
            {
                if (temp.GetType().Equals(typeof(string)))
                {
                    ...
                    labelCumulative.Text = string.Concat(labelCumulative.Text, "[", date[0].Insert(2, "-").Insert(5, "-"), "]");
                    labelRecent.Text = string.Concat(labelRecent.Text.Replace(" ", string.Empty), "[", date[1].Insert(2, "-").Insert(5, "-"), "]");
                ...
                var name = Array.Find(Enum.GetNames(typeof(Recall.ComboBoxYield)), o => o.Contains(make.FindByName.Substring(1)));
            ...
            foreach (string name in Enum.GetNames(typeof(Recall.ButtonYield)))
            {
        ...
        public DialogResult SetStrategy(DialogResult result)
        {
            ...
            foreach (string name in Enum.GetNames(typeof(Recall.TrustYield)))
            {
                ...
                if (temp.ForeColor.Equals(Color.Maroon))
                {
                    ...
                    if (max >= min && !Recall.TrustYield.trustRecent.ToString().Equals(name))
                    {
                        min = max;
                        find = name;
                    }
