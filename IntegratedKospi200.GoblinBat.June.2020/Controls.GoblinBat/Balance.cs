...
namespace ShareInvest.Controls
{
    public partial class Balance : UserControl
    {
        ...
        public void OnRealTimeCurrentPriceReflect(double price, int quantity, Kospi200 k2)
        {
                ...
                BeginInvoke(new Action(() =>
                {
                    foreach (DataGridViewRow row in balGrid.Rows)
                    {
                        ...
                        if (row.Cells[0].Value.ToString().Contains("101"))
                        {
                            ...
                            revenue = (long)((price - purchase) * quantity * k2.TransactionMultiplier - purchase * absolute * k2.TransactionMultiplier * k2.Commission - price * absolute * k2.TransactionMultiplier * k2.Commission);
