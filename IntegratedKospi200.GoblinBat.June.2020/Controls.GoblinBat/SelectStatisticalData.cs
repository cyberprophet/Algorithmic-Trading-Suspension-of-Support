...
namespace ShareInvest.Controls
{
    public partial class SelectStatisticalData : UserControl
    {
        private void ButtonClick(object sender, EventArgs e)
        {
            ...
            {
                var temp = Enum.GetValues(typeof(Recall.NumericRecall));
        ...
        {
            string[] temp = Enum.GetNames(typeof(Recall.NumericRecall));
        ...
        {
            if (DialogResult.Cancel.Equals(TimerBox.Show(new Message().Confident, "Notice", MessageBoxButtons.OKCancel, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, 7195)))
                return false;
        ...
        public SelectStatisticalData()
        {
            ...
            foreach (string name in Enum.GetNames(typeof(Recall.ComboBoxRecall)))
            {
        ...
        public void StartProgress(Dictionary<string, int> param)
        {
            foreach (KeyValuePair<string, int> kv in param)
            {
                ...
                ((Recall.ComboBoxRecall)kv.Value).ToString().FindByName<ComboBox>(this).Items.Add(int.Parse(temp[1]).ToString("N0"));
            }
            ResumeLayout();
            Application.DoEvents();
        }
        public void OnReceiveHermes(object sender, Hermes e)
        {
            ...
            if (e.Param != null && e.Param.Length > 0)
                for (int i = 0; i < e.Param.Length; i++)
                    ((Recall.NumericRecall)i).ToString().FindByName<NumericUpDown>(this).Value = int.Parse(e.Param[i]);
        }
        public void GetStrategy(DialogResult result)
        {
            switch (result)
            {
                ...
                case DialogResult.Abort:
                    foreach (string name in Enum.GetNames(typeof(Recall.NumericRecall)))
                        name.FindByName<NumericUpDown>(this).Value = 0;
