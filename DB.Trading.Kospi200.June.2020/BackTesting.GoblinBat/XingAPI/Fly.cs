...
namespace ShareInvest.Strategy.XingAPI
{
    public class Fly : StrategicChoice
    {
        ...
        protected internal override bool SetCorrectionBuyOrder(string avg, double buy)
        {
            ...
            return oPrice > buy - Const.ErrorRate * 9 && double.TryParse(avg, out double sAvg) && buy < sAvg && sAvg - abscond < sb.Value + Const.ErrorRate && API.BuyOrder.ContainsValue(oPrice) == false && SendCorrectionOrder(oPrice.ToString("F2"), sb.Key);
        }
        protected internal override bool SetCorrectionSellOrder(string avg, double sell)
        {
            ...
            return oPrice < sell + Const.ErrorRate * 9 && double.TryParse(avg, out double bAvg) && sell > bAvg && bAvg - abscond > sb.Value - Const.ErrorRate && API.SellOrder.ContainsValue(oPrice) == false && SendCorrectionOrder(oPrice.ToString("F2"), sb.Key);
        }
