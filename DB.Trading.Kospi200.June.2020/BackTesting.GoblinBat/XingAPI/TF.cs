...
namespace ShareInvest.Strategy.XingAPI
{
    public class TF : CallUpBasicInformation
    {
        ...
        bool GetCheckOnTime(string time)
        {
            ...
            if ((onTime.Substring(0, 4).Equals(Check) || string.IsNullOrEmpty(Check) || onTime.Equals(end) || time.Substring(6).Equals(TF.onTime) || time.Substring(6).Equals(nTime)) && DateTime.TryParseExact(onTime, format, CultureInfo.CurrentCulture, DateTimeStyles.None, out DateTime date))
            {
