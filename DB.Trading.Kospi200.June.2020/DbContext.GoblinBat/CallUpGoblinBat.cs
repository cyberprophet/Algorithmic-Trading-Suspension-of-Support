...
namespace ShareInvest.GoblinBatContext
{
    public class CallUpGoblinBat
    {
        ...
        protected List<ImitationGames> GetBestStrategyRecommand(List<ImitationGames> games)
        {
            try
            {
                List<ImitationGames> accumulative, cumulative;
                using (var db = new GoblinBatDbContext(key))
                {
                    accumulative = db.Games.OrderByDescending(o => o.Statistic).Take(5730).AsNoTracking().ToList();
                    cumulative = db.Games.OrderByDescending(o => o.Cumulative).Take(5730).AsNoTracking().ToList();
                }
                foreach (var game in accumulative.OrderByDescending(o => o.Cumulative))
                    using (var db = new GoblinBatDbContext(key))
                    {
                        ...
                        if (string.IsNullOrEmpty(date) == false && string.IsNullOrEmpty(oldest) == false && date.Equals(oldest) == false && DateTime.TryParseExact(date, recent, CultureInfo.CurrentCulture, DateTimeStyles.None, out DateTime rc) && DateTime.TryParseExact(oldest, recent, CultureInfo.CurrentCulture, DateTimeStyles.None, out DateTime oc) && game.Assets * 0.005 < check.Where(o => o.Date.Equals(date)).Sum(o => o.Cumulative + o.Unrealized) / (rc - oc).TotalDays)
                ...
                foreach (var game in cumulative.OrderByDescending(o => o.Statistic))
                    using (var db = new GoblinBatDbContext(key))
                    {
                        var check = db.Games.Where(o => o.Assets == game.Assets && o.Code.Equals(game.Code) && o.Commission == game.Commission && o.MarginRate == game.MarginRate && o.Strategy.Equals(game.Strategy) && o.RollOver.Equals(game.RollOver) && o.BaseTime == game.BaseTime && o.BaseShort == game.BaseShort && o.BaseLong == game.BaseLong && o.NonaTime == game.NonaTime && o.NonaShort == game.NonaShort && o.NonaLong == game.NonaLong && o.OctaTime == game.OctaTime && o.OctaShort == game.OctaShort && o.OctaLong == game.OctaLong && o.HeptaTime == game.HeptaTime && o.HeptaShort == game.HeptaShort && o.HeptaLong == game.HeptaLong && o.HexaTime == game.HexaTime && o.HexaShort == game.HexaShort && o.HexaLong == game.HexaLong && o.PentaTime == game.PentaTime && o.PentaShort == game.PentaShort && o.PentaLong == game.PentaLong && o.QuadTime == game.QuadTime && o.QuadShort == game.QuadShort && o.QuadLong == game.QuadLong && o.TriTime == game.TriTime && o.TriShort == game.TriShort && o.TriLong == game.TriLong && o.DuoTime == game.DuoTime && o.DuoShort == game.DuoShort && o.DuoLong == game.DuoLong && o.MonoTime == game.MonoTime && o.MonoShort == game.MonoShort && o.MonoLong == game.MonoLong).AsNoTracking();
                        var date = check.Max(o => o.Date);
                        ...
                        if (string.IsNullOrEmpty(date) == false && string.IsNullOrEmpty(oldest) == false && date.Equals(oldest) == false && DateTime.TryParseExact(date, recent, CultureInfo.CurrentCulture, DateTimeStyles.None, out DateTime rc) && DateTime.TryParseExact(oldest, recent, CultureInfo.CurrentCulture, DateTimeStyles.None, out DateTime oc) && check.Where(o => o.Date.Equals(date)).First().Statistic > (rc - oc).TotalDays / 7)
                            games.Add(new ImitationGames
                            {
                                Assets = game.Assets,
                                Code = game.Code,
