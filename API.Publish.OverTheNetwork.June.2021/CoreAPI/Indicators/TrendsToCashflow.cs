...
namespace ShareInvest.Indicators
{
	class TrendsToCashflow
	{
		...
		internal void OnReceiveDrawChart(object sender, SendConsecutive e)
		{
			...
			if (e.Volume != 0 && e.Date.Length > 8 && Short.Count > 1 && Long.Count > 1)
			{
				...
				{
					if (Quantity > tc.ReservationQuantity - 1 && (Offer ?? int.MaxValue) < e.Price
					...
					else if (Quantity > tc.TradingQuantity - 1 && OrderNumber.Any(o => o.Key[0] == '2' && o.Value == e.Price - Base.GetQuoteUnit(e.Price, Market)))
					{
					...
					else if (OrderNumber.Any(o => o.Key.StartsWith("1") && o.Value == e.Price + Base.GetQuoteUnit(e.Price, Market)))
					{
						...
						&& e.Price > Trend.Peek() * (1 + tc.PositionRevenue) && e.Price > (Purchase ?? 0D)
				...
				else if (date.CompareTo(Base.Transmit) > 0 && Strategics is Catalog.TrendsToCashflow cf)
				{
					...
					if (cf.ReservationQuantity > 0 && Quantity > cf.ReservationQuantity - 1)
					{
						var stock = Market;
						int quantity = Quantity / cf.ReservationQuantity, price = e.Price, sell = (int)((Purchase ?? 0D) * (1 + cf.ReservationRevenue)),
							buy = (int)((Purchase ?? 0D) * (1 - cf.Addition)), upper = (int)(price * 1.3), lower = (int)(price * 0.7),
