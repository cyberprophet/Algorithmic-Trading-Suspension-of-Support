...
{
	class Opt50068 : TR, ISendSecuritiesAPI<SendSecuritiesAPI>
	{
		internal override void OnReceiveTrData(_DKHOpenAPIEvents_OnReceiveTrDataEvent e)
		{
			if (int.TryParse(e.sPrevNext, out int next))
			{
				...
				{
					var confirm = param[2].Substring(0, 8);

					if (confirm.CompareTo(date) > 0)
						this.confirm.Enqueue(new Stocks
						{
							Code = code,
							Date = param[2].Substring(2, 6),
							Price = param[4],
							Retention = param[5]
						});
					else if (confirm.CompareTo(date) < 0)
						next = 0;
				}
				if (next > 0)
				{
		...
		const string code = "opt50068";
		const string id = "종목코드;기준일자";
		const string rq_name = "옵션일차트요청";
		readonly string[] op_mutiple = { "현재가", "누적거래량", "일자", "시가", "고가", "저가", "전일종가" };
