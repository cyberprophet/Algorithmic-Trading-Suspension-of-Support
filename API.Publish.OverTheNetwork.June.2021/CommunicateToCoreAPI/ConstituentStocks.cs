        ...
        public async Task<Stack<Catalog.IncorporatedStocks>> GetConstituentStocks(int market, DateTime now)
        {
            try
            {
                ...
                var response = await client.ExecuteAsync(new RestRequest(Security.Contents[3], Method.POST)
                    .AddHeader(Security.content_type, Security.Contents[4])
                    .AddParameter(Security.Contents[5], market)
                    .AddParameter(Security.Contents[6], security.GetMarket(market))
                    .AddParameter(Security.Contents[7], Security.Contents[8])
                    .AddParameter(Security.Contents[9], now.ToString(format))
                    .AddParameter(Security.Contents[0xA], (await client.ExecuteAsync(new RestRequest(Security.Contents[0xB], Method.GET))).Content), source.Token);
