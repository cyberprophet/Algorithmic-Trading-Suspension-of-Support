...
{
    class T8401 : Query, IQuerys<SendSecuritiesAPI>
    {
        protected internal override void OnReceiveData(string szTrCode)
        {
            ...
            {
                var distinct = string.Concat(code[i].Substring(0, 3), code[i].Substring(5));

                if ((string.IsNullOrEmpty(temp) || temp.Equals(distinct) == false) && code[i].StartsWith("1"))
                {
                    temp = distinct;
                    Send?.Invoke(this, new SendSecuritiesAPI(new StringBuilder(code[i]).Append(';').Append(code[1 + i])));
                }
            }
        }
