...
{
    class T8430 : Query, IQuerys<SendSecuritiesAPI>
    {
        protected internal override void OnReceiveData(string szTrCode)
        {
            var enumerable = GetOutBlocks();
            string[] code = null, name = null, price = null, gubun = null;
            ...
            while (enumerable.Count > 0)
            {
                ...
                for (int i = 0; i < length; i++)
                    switch (enumerable.Count)
                    {
                        ...
                        case 0:
                            gubun[i] = GetFieldData(param.Block, param.Field, i);
                            break;
                    }
            }
            Send?.Invoke(this, new SendSecuritiesAPI(new Tuple<string[], string[], string[], string[]>(code, name, gubun, price)));
        }
