...
namespace ShareInvest.Analysis
{
    class Consecutive
    {
        ...
        internal Consecutive(TrendsInValuation strategics, Holding ho)
        {
            Short = new Stack<double>();
            Long = new Stack<double>();
            Trend = new Stack<double>();
        ...
        void OnReceiveDrawChart(object sender, SendConsecutive e)
        {
            ...
            switch (strategics)
            {
                case SatisfyConditionsAccordingToTrends _:
                    tShort = sc.Short;
                    tLong = sc.Long;
                    trend = sc.Trend;
            ...
            if (e.Volume != 0 && e.Date.Length != 8 && Short.Count > 1 && Long.Count > 1)
            {
                ...
                switch (sender)
                {
                    case OpenAPI.HoldingStocks os:
                        os.OnReceiveTrendsInPrices(e, gap, strategics.Code.Length == 6 ? Trend.Peek() : tMinute);
                        break;
        ...
        readonly SatisfyConditionsAccordingToTrends sc;
        readonly TrendToCashflow tc;
        readonly TrendsInValuation tv;
