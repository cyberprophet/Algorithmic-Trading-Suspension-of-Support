...
namespace ShareInvest.Analysis
{
    ...
    {
        public static T FindByCode<T>(this object target, string name) where T : class
            => target.GetType().GetProperty(name, BindingFlags.Instance | BindingFlags.Public).GetValue(target) as T;
        public static T FindByCode<T>(this string name, object target) where T : class
            => target.GetType().GetProperty(name, BindingFlags.Instance | BindingFlags.Public).GetValue(target) as T;
